...
const Orders = () => {
  ...
  // Processes the cart data to create a displayable list of items
        ...
        for (const size in cartData[itemId]) {
          ...
          if (quantity > 0) {
            items.push({
              ...
              // Safely access the image to prevent errors
              image: product.image?.[0] || "", // Use optional chaining and a fallback
            });
          }
        }
      }
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
          <div className="mb-8">
            ...
            <p className="text-gray-600 text-lg">
              Track and manage your clothing orders
            </p>
          </div>
              ...
              <nav className="flex space-x-8 px-6 py-4">
                {[
                  { key: "all", label: "All Orders", count: ordered.length },
                  {
                    key: "processing",
                    label: "Processing",
                    count: ordered.filter((o) => o.status === "processing").length,
                  },
                  {
                    key: "shipped",
                    label: "Shipped",
                    count: ordered.filter((o) => o.status === "shipped").length,
                  },
                  {
                    key: "delivered",
                    label: "Delivered",
                    count: ordered.filter((o) => o.status === "delivered").length,
                  },
                ].map((tab) => (
                  <button
                    key={tab.key}
